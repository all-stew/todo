...
const DB = {
  ...
  insert(key, value) {
    return db.read().get(key).insert(value).write()
  },
  removeById(key, id) {
    return db.read().get(key).removeById(id).write()
  },
  groupby(key, prop) {
    const d = db.read().get(key).sortBy(prop).reverse().groupBy(prop).value()
    return d
  }
}
export default DB
